...
export default function HomePage() {
  ...
  const { entries, removeEntry, editEntry } = useFoodLog(selectedDate);
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{formatDateSub(selectedDate)}</p>
          </div>
          <button
            onClick={goForward}
            disabled={isToday}
            className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            aria-label="Next day"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
