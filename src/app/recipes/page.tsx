    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Homemade, chocolate flavour"
        />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {([
            ...
            ['sugar', 'Sugar (g)'],
            ['sodium', 'Sodium (mg)'],
          ] as [keyof Nutrients, string][]).map(([key, label]) => (
            <div key={key}>
  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch('/api/analyze-food', {
        ...
        body: JSON.stringify({ method: 'text', text }),
      });
      ...
      if (data.success && data.entry) {
        setPreview({
          ...
          nutrients: data.entry.nutrients,
          createdAt: Date.now(),
        });
      } else {
        setError(data.error || 'Could not analyze recipe');
      }
    } catch {
          ...
          <div className="grid grid-cols-2 gap-2 mt-3">
            {Object.entries(preview.nutrients).map(([k, v]) => (
              <div key={k} className="bg-white rounded-lg p-2">
                ...
                <p className="text-sm font-semibold text-gray-900">{Math.round(v)}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea
        ...
        placeholder="e.g. Homemade protein bar: 30g oats, 20g whey protein, 15g peanut butter, 10g honey..."
      />
      {error && <p className="text-sm text-red-500">{error}</p>}
      <button
        onClick={analyze}
        disabled={loading || !text.trim()}
        className="w-full py-3 rounded-xl bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors disabled:opacity-40 flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
...
function ImageForm({ onSave }: { onSave: (r: Recipe) => void }) {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        const res = await fetch('/api/analyze-food', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ method: 'image', imageBase64: base64, imageMime: mime }),
        });
        const data = await res.json();
        if (data.success && data.entry) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-gray-500">Upload a photo of your recipe or meal and AI will break down the nutrition.</p>
...
export default function RecipesPage() {
  ...
  return (
    <main className="max-w-md mx-auto px-4 pt-6 pb-24">
      ...
      {tab === 'list' && (
        <div className="space-y-3">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {recipe.amount && recipe.unit && (
                        <p className="text-xs text-gray-400 mt-0.5">{recipe.amount} {recipe.unit}</p>
                      )}
                    </div>
          ...
          <button
            ...
            className="w-full py-3 rounded-xl border-2 border-dashed border-gray-200 text-sm font-medium text-gray-400 hover:border-green-400 hover:text-green-500 transition-colors"
          >
            + Add new recipe
          </button>
        </div>
      ...
      {tab === 'manual' && (
        <Card>
          <h3 className="font-semibold text-gray-900 mb-4">Enter values manually</h3>
      ...
      {tab === 'ai' && (
        <Card>
          <h3 className="font-semibold text-gray-900 mb-4">Analyze with AI</h3>
      ...
      {tab === 'image' && (
        <Card>
          <h3 className="font-semibold text-gray-900 mb-4">Analyze from image</h3>
